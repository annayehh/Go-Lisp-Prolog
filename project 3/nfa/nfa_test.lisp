...
(define-test test-reachable
    (assert-equal '(0) (reachable 'fooTransitions 0 0 NIL))
    (assert-equal NIL (reachable 'fooTransitions 0 3 '(A B C)))
    (assert-equal '(0 1 3) (reachable 'fooTransitions 0 3 '(A B)))
    (assert-equal NIL (reachable 'fooTransitions 0 3 '(A A A)))
    (assert-equal '(0 2 3) (reachable 'fooTransitions 0 3 '(A C)))

    (assert-equal '(0) (reachable 'expTransitions 0 0 NIL))
    (assert-equal NIL (reachable 'expTransitions 0 2 '(B B)))
    (assert-equal '(0 1 0 1) (reachable 'expTransitions 0 1 '(A B A)))
    (assert-equal NIL (reachable 'expTransitions 0 2 '(A B)))
    (assert-equal '(0 1 0 2) (reachable 'expTransitions 0 2 '(A B A)))

    (assert-equal '(0) (reachable 'langTransitions 0 0 NIL))
    (assert-equal '(0 1) (reachable 'langTransitions 0 1 '(B)))
    (assert-equal '(0 0 0 0) (reachable 'langTransitions 0 0 '(A A A)))
    (assert-equal NIL (reachable 'langTransitions 0 1 '(A B B A)))
    (assert-equal '(0 1 1 0 0 1) (reachable 'langTransitions 0 1 '(B A B A B)))
)
;;;;;;;;;;;;;;;;;
;;; run tests ;;;
;;;;;;;;;;;;;;;;;
